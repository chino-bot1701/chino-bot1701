# José Paz Rangel Rojas
### Analista de Datos y Consultor Estratégico

Profesional enfocado en la transformación de datos crudos en decisiones estratégicas. Actualmente aplico modelos de análisis estadístico para optimizar procesos en el sector inmobiliario y consultoría privada.

---

### Áreas de Especialización
* **Análisis Predictivo:** Implementación de modelos para la estimación de valores en el mercado inmobiliario (Inmobiliaria ALTEA).
* **Integración de Sistemas:** Estudio y manejo de arquitecturas de datos y APIs financieras.

---

### Competencias Técnicas
* **Lenguajes:** Python, SQL, R.
* **Visualización:** Power BI, Matplotlib.
* **Entornos:** Git, GitHub, VS Code.

---

### Experiencia y Proyectos
* **Proyecto Inmobiliaria ALTEA:** Desarrollo de un sistema de análisis de datos para la toma de decisiones estratégicas.
* **Consultoría Externa:** Colaboración en proyectos de análisis de datos para optimización de negocios.
* **Investigación de APIs:** Implementación y consumo de servicios bancarios para análisis financiero.

---

### Contacto
* Perfil de LinkedIn: https://www.linkedin.com/in/jos%C3%A9-paz-rangel-rojas-9a49b7334
* Consultoría: http://soltecmty.com

*"Resiliencia y consistencia en el desarrollo de soluciones basadas en datos."*
